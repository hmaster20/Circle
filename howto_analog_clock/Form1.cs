...
namespace howto_analog_clock
{
    ...
    {
        // csharphelper.com/blog/2016/08/draw-an-analog-clock-in-c/
        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            ...
            // Draw the center. (кружок в центре)
        ...
        {
            // Draw.
            using (Pen thick_pen = new Pen(Color.Gray, 4))
            {
                // Outline.
                ...
                for (int minute = 1; minute <= 60; minute++)
                {
                    ...
                    else
                    {
                        ...
                        gr.DrawLine(Pens.Green, inner_pt, outer_pt);
                    }
                    if (minute % 15 == 0)   //отметки каждые 3 часа
                    {
                        PointF inner_pt = new PointF(big_x_factor * cos_angle, big_y_factor * sin_angle);
                        //Pen newPen = (Pen)thick_pen.Clone();
                        //newPen.Color = Color.Black;                        
                        //gr.DrawLine(newPen, inner_pt, outer_pt);
                        gr.DrawLine(new Pen(Color.Black, 4), inner_pt, outer_pt);
                    }
                }
            }
        }

        // Show the time digitally. (электронные часы в центре)
            ...
            if (ShowDigital)
            {
                    ...
                    ClientSize.Height / 10, GraphicsUnit.Pixel))
                {
                    using (StringFormat string_format = new StringFormat())
                    {
                        ...
                        gr.DrawString(DateTime.Now.ToLongTimeString(), font, Brushes.Gray, 0, -font.Height, string_format);
                    }
                }
            }
        }

        // Draw the clock's hands. (Стрелочки)
        private void DrawClockHands(Graphics gr)
        {
            using (Pen thick_pen = new Pen(Color.Red, 4))
            {
                ...
                thick_pen.Color = Color.DarkGray;    // Цвет часовой и минутной
                ...
                // Draw the second hand. (Секундная стрелочка)
                ...
                gr.DrawLine(Pens.Green, second_pt, center);
            }
        }
